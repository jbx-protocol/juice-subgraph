...
import { address_shared_textResolver } from "../../contractAddresses";
...
export function handleTextChanged(event: TextChanged): void {
  if (event.params.key != TEXT_KEY || !address_shared_textResolver) return;

  // Get value of text record
  const textResolver = TextResolver.bind(
    Address.fromString(address_shared_textResolver!)
  );
