...
export function handleDelegateDeployed(event: DelegateDeployed): void {
  ...
  JB721Delegate3_4DataSource.createWithContext(
    address,
  ...
  if (!address_shared_jbTiered721DelegateStore3_3) {
    log.error(
      "[jbTiered721DelegateDeployer_3_4:handleDelegateDeployed] missing address_shared_jbTiered721DelegateStore3_3",
      []
    );
    return;
  }
  ...
  if (maxTierCall.reverted) {
    ...
    log.error(
      "[jbTiered721DelegateDeployer_3_4:handleDelegateDeployed] maxTier() reverted for {}",
      [address.toHexString()]
    );
    return;
  }
